			...
			<div class="col-lg-8">
				...
				<h2 class="page-header"><span style="color: #ff52a0;">IZONE</span> 자유 게시판
					...
					<table class="table table-bordered table-hover">
						...
						<!-- articles에서 하나씩 꺼내서 article에 담는다.
						article은 BoardVO가 된다.  -->
						<c:forEach var="article" items="${articles}">
						<tr style="color: #ff52a0;">
							<!-- 게시글리스트가 들어갈 행 getter를 안써도 된다.-->
							<td>${article.bId}</td>
							<td>${article.bName}</td>
							
							<td>
							<%-- 글내용 보여달라는 request를 controller로 보내야한다.  --%>
								<a href="/izone/board/bContent.izone?bId=${article.bId}" style="color: orange">
							${article.bTitle}
							</a>
							</td>
							<td>
								<fmt:formatDate pattern="yyyy년 MM월 dd일 hh시 mm분" value="${article.bDate}"/>
							</td>
							<td>${article.bHit}</td>
						</tr>		
						</c:forEach>				
